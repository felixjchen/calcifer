...
export class FileDataSource extends DataSource<FileFlatNode> {
    ...
    constructor(private _treeControl: FlatTreeControl<FileFlatNode>, private _fileStore: FileStoreService) {
        super();
    }
    ...
    update(files: FileNode[]): void {
        ...
        currentData.forEach(flatNode => this.flatNodeByPath.set(flatNode.path, flatNode));
    ...
    connect(collectionViewer: CollectionViewer): Observable<FileFlatNode[]> {
        ...
        this._subscriptions.push(changed.subscribe((change: SelectionChange<FileFlatNode>) => {
        ...
        return merge(...changes).pipe(
            map(() => {
                this.expandedData.next(this._treeFlattener.expandFlattenedNodes(this.data, this._treeControl));
    ...
    private _toggleNode(node: FileFlatNode, expand: boolean): void {
        const foundIndex = this.data.findIndex(nodeToFind => nodeToFind.path === node.path);
        ...
        this._fileStore.getDirectoryByPath(node.original.path, (children: FileNode[]) => {
    ...
    updateRootChildren(children: any): void {
        ...
        currentChildren.forEach(c => {
            const foundIndex = this.data.findIndex(nodeToFind => nodeToFind.path === c.path);
            if (foundIndex !== -1) {
        ...
        currentChildren = currentChildren.filter(fileFlatNode => fileFlatNode !== null);
