...
export const get_container_start_command = (id) => {
  if (production) {
    return `docker run --runtime=sysbox-runc -d --network project-calcifer_default --name=${id} --network-alias=${id} -e LETSENCRYPT_HOST=${id} -e VIRTUAL_HOST=${id} felixchen1998/calcifer-playground:latest`;
  } else {
